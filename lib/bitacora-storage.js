...
async function guardarEvidencia(message, fechaArchivo, carpetaBase = 'evidencias') {
    if (!message.hasMedia) {
        return '';
    }
...
async function guardarActividad(pool, data) {
    if (data.grupo !== 'BITACORA-MTTO-SHP1') {
        console.log('⛔ Insert bloqueado en actividades_mtto (grupo no BITACORA):', data.grupo);
        return null;
    }
